...
>
> 生成时间：2026-09-14 21:46:37 ｜ 数据源：migration/tasks.json（schema=1，updated=2026-09-14）

## 总览

**总任务数 28** ｜ pending(未完成)=27、in_progress(进行中)=0、partial(部分完成)=0、unverified(未验证)=0、done(已完成)=1 ｜ **完成率 4%**（权重：done=1，in_progress/partial/unverified=0.5，pending=0）
...
| M0.2 | 基线构建跑通 | pending（未完成） | ./gradlew build 退出码 0，CI baseline-build job 变绿 | — |
...
| M1.2 | client 源集分离 | pending（未完成） | client 类移入专用源集，@OnlyIn(Dist.CLIENT) 从基线 267 处显著下降，具体数字记录在 evidence | — |
...
| M2.4 | 注册层迁移 | pending（未完成） | RegistrationHelper/BC*Blocks 等 3 处注册基类重写为 NeoForge DeferredRegister，RegistryObject 引用（基线 38 文件 362 处）全部改为 DeferredHolder 并清零 | — |
...
采集时间：2026-09-14 21:46:37；采集范围：仓库根目录（排除 .git、.gradle、build、buildcraft_resources_generated）。
