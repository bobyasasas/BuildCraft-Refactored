...
>
> 生成时间：2026-09-15 00:41:09 ｜ 数据源：migration/tasks.json（schema=1，updated=2026-09-15）

## 总览

**总任务数 28** ｜ pending(未完成)=21、in_progress(进行中)=0、partial(部分完成)=1、unverified(未验证)=0、done(已完成)=6 ｜ **完成率 23%**（权重：done=1，in_progress/partial/unverified=0.5，pending=0）
...
| M1.1 | Gradle 多项目拆分 | partial（部分完成） | settings.gradle 正式 include BuildCraftAPI/expression/主体模块，各模块可独立构建，srcDir 拼装方式移除 | evidence：:expression 已拆为纯 java-library 子项目（64 文件 0 forge import），:expression:test 58 个测试全绿独立可构建，根 build 退出码0，jar 内 expression class 数量拆分前后一致（431 个）；剩余：:buildcraftapi 拆分 + 全部 srcDir 拼装移除 |
...
采集时间：2026-09-15 00:41:09；采集范围：仓库根目录（排除 .git、.gradle、build、buildcraft_resources_generated）。
