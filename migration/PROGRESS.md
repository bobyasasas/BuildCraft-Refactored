...
>
> 生成时间：2026-09-15 08:10:11 ｜ 数据源：migration/tasks.json（schema=1，updated=2026-09-15）

## 总览

**总任务数 28** ｜ pending(未完成)=13、in_progress(进行中)=0、partial(部分完成)=0、unverified(未验证)=0、done(已完成)=15 ｜ **完成率 54%**（权重：done=1，in_progress/partial/unverified=0.5，pending=0）
...
| Phase 2 分层迁移到 MC 26.1.2 + NeoForge 26.1.2.109 | 9 | 4 | 44% |
...
采集时间：2026-09-15 08:10:11；采集范围：仓库根目录（排除 .git、.gradle、build、buildcraft_resources_generated）。

| 指标 | 当前值 | 调研基线(2026-09) | 目标 |
|---|---:|---:|---|
| .java 文件总数 | 1,829 | 1772（main 1510 + API 子模块 262） | 无目标(参考) |
| .java 总行数 | 188,186 | — | 无目标(参考) |
| Forge import 文件数 | 549 | 549 | 0 |
| Forge import 出现次数 | 1,408 | — | 0 |
| TODO 出现次数 | 173 | 222 | 随 M1.4 下降 |
| FIXME 出现次数 | 23 | 24 | 随 M1.4 下降 |
| @OnlyIn(Dist.CLIENT) 出现次数 | 250 | 279 | 随 M1.2 下降 |
| RegistryObject 涉及文件数 | 39 | 42 | 0 |
